...
def FedAvg(clients, schedule, device, num_classes, model):
    if model == 'Net':
        weighted_averaged_model = Net(num_classes)
    elif model == 'ResNet50':
        weighted_averaged_model = ResNet50(num_classes)
    elif model == 'ResNet18':
    ...
    total_samples_number = total_samples_num(clients, schedule)
    for client in clients:
        if client.client_id in schedule[clients[0].client_id] and client.client_id in schedule:
...
def total_samples_num(clients, schedule):
    total = 0
    for client in clients:
        if client.client_id in schedule[clients[0].client_id] and client.client_id in schedule:
